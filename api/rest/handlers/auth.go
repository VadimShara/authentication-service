...
func (r *Resolver) login(w http.ResponseWriter, req *http.Request) {
	...
	refreshToken, err := r.authService.CreateRefreshToken(context.Background(), model.Username, role)
	if err != nil {
		switch {
	...
	r.authService.SaveRefreshToken(context.Background(), model.Username, refreshToken)
	if err != nil {
		webutil.SendJSONResponse(w, http.StatusInternalServerError, web.ErrorResponse(core.InternalErrorCode, nil, nil))
			return
	}

	accessToken, err := r.authService.CreateAccessToken(context.Background(), model.Username, role)
	if err != nil {
		switch {
		case errors.Is(err, core.ErrUsernameExists):
			webutil.SendJSONResponse(w, http.StatusConflict, web.ErrorResponse(core.UsernameIsTaken, nil, nil))
	...
	webutil.SendJSONResponse(w, http.StatusOK, web.OKResponse(core.JWTRecievedCode, accessToken, nil))
}
