...
type (
	Service interface {
		...
		CreateAccessToken(context.Context, string) (jwt.AccessToken, error)
		CreateRefreshToken(context.Context, string) (jwt.RefreshToken, error)
...
func (s *serviceImpl) CreateAccessToken(ctx context.Context, username, role string) (jwt.Token, error) {
	return s.jwtService.CreateAccessToken(ctx, jwt.UserData{Username: username, Role: role})
}

func (s *serviceImpl) CreateRefreshToken(ctx context.Context, username, role string) (jwt.Token, error) {
	return s.jwtService.CreateRefreshToken(ctx, jwt.UserData{Username: username, Role: role})
}
...
func (s *serviceImpl) SaveRefreshToken(ctx context.Context, username string, refreshToken string) error {
	return s.usersStore.SaveRefreshToken(context.Background(), username, refreshToken)
}